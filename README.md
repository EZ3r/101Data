...
graph TD
    ...
    subgraph Inference [阶段四：在线检测推理流程 (新数据)]
        ...
        NewTransform --> NewSlidingWindow(滑动窗口构建新序列):::process
        ...
        EncoderInferNew --> NewLatentVecs[新潜在向量 (N_test, Latent_Dim)]:::data

        NewLatentVecs --> GMMScoreNew(GMM 打分 (计算 Log-Likelihood)):::process
        TrainedGMM -. 加载模型 .-> GMMScoreNew
        GMMScoreNew --> NewScores[新数据健康分数]:::data
